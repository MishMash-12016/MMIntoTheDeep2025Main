        ...
        if (distanceX == -999) {
            ...
            if (!longPID) {  // If long PID was never triggered, use short
                if (distanceX >= min && distanceX <= max) {
                    drive.setDrivePowers(new PoseVelocity2d(new Vector2d(0, 0), pidControllerShort.calculate(distanceX) / voltageSensor.getVoltage()));
                    shortPID = true;
                } else {  // If the distance is outside the min-max range at the start, use long
                    drive.setDrivePowers(new PoseVelocity2d(new Vector2d(0, 0), pidControllerLong.calculate(distanceX) / voltageSensor.getVoltage()));
                    longPID = true;
                }
